...
module.exports = function code(input, output, options) {
  ...
  var allfiledata = {};
  return Promise.all([cliUtil.scanFileList(path.resolve(input), null, options.isRecurse), readTranslateDBFilePromise, options.inputPOFile && i18ncUtil.loadPOFile(path.resolve(options.inputPOFile)), options.inputPODir && i18ncUtil.autoLoadPOFiles(path.resolve(options.inputPODir))]).then(function (data) {
    var fileInfo = data[0];
    ...
    if (fileInfo.type == 'list') {
      ...
      return cliUtil.file2i18nc(file, myOptions).then(function (data) {
        ...
        debug('writefile: %s', wfile);
        return cliUtil.getWriteOneFilePath(wfile, file).then(function (wfile) {
          return fs.writeFileAsync(wfile, code).then(function () {
            console.log('write file: ' + wfile);
          });
        });
      });
