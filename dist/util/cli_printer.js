...
function printRefs(info, paddingLeft) {
  ...
  var mainTableData = [[exports.colors.gray('type'), '' + info.type], [exports.colors.gray('fileKey'), info.fileKey || emptySymbol], [exports.colors.gray('subtype'), info.subtype || emptySymbol], [exports.colors.gray('joinIndexs'), info.joinIndexs && info.joinIndexs.join(',') || emptySymbol]];
...
function _printWordListTable(list, firstColumnStyle) {
  var mainTableData = list.map(function (item) {
    return [item.loc ? exports.colors.gray(item.loc) : ' ', item.value ? exports.colors.yellow(item.value) : ' ', item.reason || ' '];
  }).filter(function (arr) {
...
function _printTable(mainTableData, firstColumnStyle) {
  ...
  if (table.table) return table.table(mainTableData, tableOptions);else return table.default(mainTableData, tableOptions);
}
